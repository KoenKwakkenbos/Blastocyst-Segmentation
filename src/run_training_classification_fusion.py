...
from networks.model import build_resnet50, transfer_model, trainable_model, model_rad, small_cnn, rdunet_features, fusion_model
...
# CLIN_VARS = ['HFEA_age_cat', 'age', 'insem', 'e_count', 'e_d1_2pn', 'e_util']
CLIN_VARS = ['HFEA_age_cat', 'age', 'insem', 'e_count', 'e_d1_2pn', 'e_util', 'ratio_util']

# CONT_COLS = ['max_vol', 'avg_vol', 'tsb', 'dev_time', 'avg_growth_rate', 'max_growth_rate', 'max_collapse', \
#             'max_duration', 'mean_duration', 'mean_collapse', 'exponential_1', 'exponential_2', 'hist_1', 'hist_2',\
#             'hist_3', 'hist_4', 'hist_5', 'bin_1', 'bin_2', 'bin_3', 'bin_4', 'bin_5', 'age', 'e_count', 'e_d1_2pn', 'e_util']

CONT_COLS = ['max_vol', 'avg_vol', 'tsb', 'dev_time', 'avg_growth_rate', 'max_growth_rate',\
            'overall_growth', 'init_growth_rate', 'final_growth_rate', 'time_max_growth_rate', 'rel_time_max_growth_rate',\
            'exponential_1',  'bin_1', 'bin_2', 'bin_3', 'age', 'e_count', 'e_d1_2pn', 'e_util', 'ratio_util']

# CAT_COLS = ['HFEA_age_cat', 'insem']
CAT_COLS = ['insem', 'HFEA_age_cat']

BIN_COLS = ['male', 'hist_5']
...
def main(): 
    ...
    for fold in range(experiment['n_folds']):
        ...
        feature_df = pd.read_csv(args.expansion, index_col=0)

        X_feat = feature_df[CONT_COLS + CAT_COLS + BIN_COLS]
        X_feat = pd.get_dummies(X_feat, columns=CAT_COLS)

        
        scaler = StandardScaler()

        X_feat_train = X_feat.loc[train_ids]
        X_feat_train[CONT_COLS] = scaler.fit_transform(X_feat_train[CONT_COLS])

        X_feat_val = X_feat.loc[test_ids]
        X_feat_val[CONT_COLS] = scaler.transform(X_feat_val[CONT_COLS])
        ...
        model = fusion_model(input_shape=(800, 800, 1), base_model=experiment['model'], feature_size=29)
