    ...
    for i in range(n): # Loop over each mask
        ...
        if len(regions) > 1: # If there are more than one region
            for rg in regions[1:]: # Loop over the smaller regions
                labels_mask[rg.coords[:,0], rg.coords[:,1], i] = 0 # Set their labels to zero
        labels_mask[labels_mask[:, :, i] != 0, i] = 1 # Set the remaining labels to one
        mask[:, :, i] = labels_mask[:, :, i] # Update the mask with the labels
        
        if np.max(mask[:, :, i]) == 255: # If the mask values are 255
            mask[:, :, i] = mask[:, :, i] / 255 # Normalize them to 0 or 1
        im_flood_fill = mask[:, :, i].copy() # Copy the mask for flood fill
        overlay = np.zeros((h + 2, w + 2), np.uint8) # Create an overlay with extra padding
